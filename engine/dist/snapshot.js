...
export function buildSnapshot(kind, raw, warnings) {
    const warn = (w) => {
        warnings.push(Object.freeze(w));
    };
    const rows = (table) => raw[table] ?? [];
    // ── shared by both artifacts ──────────────────────────────────────────────
    const metaRow = rows("cartridge_meta")[0];
    if (!metaRow)
        throw new CartridgeError("meta_invalid", "cartridge_meta has no row", { table: "cartridge_meta" });
    const meta = Object.freeze({
        cartridgeKind: str(metaRow, "cartridge_kind"),
    ...
    const days = rows("project_days").map((r) => Object.freeze({ id: num(r, "id"), day: str(r, "day"), startTime: num(r, "start_time"), endTime: num(r, "end_time") }));
    ...
    for (const r of rows("media_item")) {
        ...
        for (const [column, fileId] of [["portrait_file_id", item.portraitFileId], ["landscape_file_id", item.landscapeFileId]]) {
    ...
    for (const r of rows("media_file_variant")) {
        ...
        if (list)
            ...
            variantLists.set(v.mediaFileId, [v]);
    }
    const variants = new Map();
    for (const [fileId, list] of variantLists)
        variants.set(fileId, Object.freeze(list.sort((a, b) => a.id - b.id)));
    const base = {
        meta,
        project,
    ...
    if (kind === "project") {
        return Object.freeze({ kind: "project", ...base, warnings: Object.freeze(warnings) });
    }
    ...
    const surfaceConfig = Object.freeze({
        id: num(c, "id"),
        name: str(c, "name"),
    ...
    const lanes = { portrait: [], landscape: [], demo_station: [] };
    ...
    for (const lane of Object.values(lanes))
        lane.sort((a, b) => a.timestamp - b.timestamp || a.id - b.id);
    ...
    for (const r of rows("session_set_entry")) {
        ...
        if (list)
            ...
            setEntryLists.set(e.sessionSetId, [e]);
    }
    const sessionSetEntries = new Map();
    ...
    for (const r of playlistRows)
        playlistNames.set(num(r, "id"), str(r, "name"));
    const entryLists = new Map();
    for (const id of playlistNames.keys())
        entryLists.set(id, []);
    ...
    const byTime = (a, b) => a.timestamp - b.timestamp || a.id - b.id;
...
function num(r, column) {
    return r[column];
}
function numOrNull(r, column) {
    return (r[column] ?? null);
}
function str(r, column) {
    return r[column];
}
function strOrNull(r, column) {
    return (r[column] ?? null);
}
function bool(r, column) {
    return r[column] === true;
}
function parseJson(text, table, column, rowId, warn) {
...
function jsonArray(r, table, column, rowId, warn) {
...
function jsonValue(r, table, column, rowId, warn) {
    const text = strOrNull(r, column);
    if (text === null)
        return null;
    return parseJson(text, table, column, rowId, warn).value;
}
function renderModes(r, rowId, warn) {
...
const DEFAULT_RENDER_MODES = Object.freeze(["simple"]);
